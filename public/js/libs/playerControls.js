...
THREE.PlayerControls = function (camera, player, domElement) {
	...
	this.init = function () {
		...
		this.camera.lookAt(11.99665355682373,5.791983604431152,4.77586030960083);
	...
	this.checkKeyStates = function () {
		...
		if (keyState[65]) {
			...
			// left arrow keyState[37] or 'a' - rotate left
			playerIsMoving = true;
		...
		if (keyState[68]) {
			...
			// right arrow keyState[39] or 'd' - rotate right
			playerIsMoving = true;
		...
		if (keyState[69] || keyState[39]) {

			// 'e' - strage right keyState[39] right arrow
			playerIsMoving = true;
